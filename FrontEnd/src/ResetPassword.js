...
function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) {
      setError('Token inválido o expirado. Solicita de nuevo el correo de recuperación.');
      return;
    }
    try {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="flex justify-center items-center min-h-screen pt-20 px-4">
        <form onSubmit={handleSubmit} className="bg-white shadow-2xl rounded-xl px-8 pt-8 pb-10 w-full max-w-md">
          <div className="text-center mb-8">
            <img src="/logo.png" alt="logo" className="w-24 mx-auto mb-4" />
            <h1 className="text-2xl font-bold text-pemex-green">Cambia tu contraseña</h1>
          ...
          <div className="mb-6">
            ...
            <div className="relative">
              <input type={showConfirmPassword ? 'text' : 'password'} inputMode="numeric" pattern="[0-9]*" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pemex-green transition-all duration-300" placeholder="123456" required />
